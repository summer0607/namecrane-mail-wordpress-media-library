...
- 可在基础配置中开启静态图片自动转换，JPG、PNG 等位图会先转为 WebP 再上传，以减少传输体积和链接等待时间；动态 GIF 与 SVG 保持原格式。
- 可按文件类型限制上传数量、大小和格式。
- 可自定义媒体库名称。
...
| 自动将图片格式转为 WebP | 默认关闭。开启后，JPG、PNG 等静态位图会先转为 WebP 再上传；已是 WebP、SVG 和动态 GIF 保持原格式。 |
