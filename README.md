...
将 **NameCrane Mail 邮箱与网盘空间**接入 WordPress 媒体库。上传的图片、视频、音频、文档和压缩包会保存在 NameCrane 文件空间中，并自动生成可插入文章的公开链接。
...
- 可选替换 WordPress 标准媒体入口；文章、页面和支持标准媒体框的主题入口都会使用同一套媒体选择窗口。
- 上传成功后可直接插入编辑器或回填外部文件地址。
- 每位用户只能查看、管理自己的文件记录；管理员可统一管理全部文件。
- 支持 FTPS 和 SFTP，支持填写 CDN 加速域名输出公开链接。
- 支持默认存储邮箱，也支持授权后让指定角色或指定用户登录自己的 NameCrane Mail 账户。
- 可按文件类型限制上传数量、大小和格式。
...
1. 在文章、页面或主题的标准媒体入口点击“上传/添加媒体”。
2. 在 NameCrane 媒体库窗口选择“上传文件”，可点击选择，也可直接拖入文件。
3. 上传完成后，在“全部文件”中选中目标文件，右下角链接会自动填入。
4. 点击“插入”，即可写入文章编辑器或当前的文件地址输入框。

“全部文件”也支持手动粘贴外部链接，再点击“插入”回填。

## 用户与上传限制

- **默认规则**：所有站点都可以设置，控制单次上传数量、各类文件的大小和允许格式。
- **设置各角色组专属限制**、**指定用户专属限制**：需获得插件授权后使用；未授权时会锁定，并自动回退到默认规则。
- **独立邮箱登录**：需获得插件授权后，可让指定角色或指定用户通过 WordPress 后台登录自己的 NameCrane Mail 账户，单独使用自己的文件空间。

默认规则不会被主题上传限制覆盖；插件会在上传前和服务端再次校验。

## 授权与更新

每个安装网站可免费试用 7 天。试用结束后，可在“设置 → 授权管理”使用支付宝获取永久授权；付款成功后，授权自动绑定当前网站域名，不需要填写授权码。

- 未授权时不能新建存储连接或上传新文件。
- 已上传文件和已有公开链接不会因授权变化被删除。
- 即使授权被撤销，仍可检查和下载安装插件更新，以便修复问题。

## NameCrane Mail 服务

NameCrane 的 CraneMail 提供自定义域名邮箱、网页邮箱、联系人与日历、文件存储与公开分享等功能。本插件利用其中的文件存储能力作为 WordPress 媒体空间。
...
根据 NameCrane 当前公开页面，250GB 生命周期套餐为一次性付费，包含邮件与云存储、最多 25 个主域名和不限邮箱账户；实际价格、套餐内容与可用地区请始终以其官网下单页为准。当前未找到 NameCrane 公开且可核实的推广返佣计划，因此本项目未放置任何未经官方确认的推广链接。

## 安全说明

- 本仓库不包含邮箱密码、默认存储账户、GitHub 更新令牌、支付宝密钥或授权记录。
- 插件只把邮箱密码加密保存在使用者自己的 WordPress 数据库中；请不要把数据库导出、设置导出或包含密码的截图提交到 Issue。
- 授权服务、订单处理和更新令牌保留在游先锋正式站的私有服务端，不在本仓库发布。
- 这是运行在站点管理员自己服务器上的 PHP 插件。管理员始终可以修改自己安装的任意插件文件；如需不可篡改的商业授权，必须采用由服务端代理上传与存储的架构。

发现安全问题请不要公开提交 Issue，请发送邮件至 [admin@youxianfeng.com](mailto:admin@youxianfeng.com)。

## 开发与反馈

欢迎提交 Issue，反馈兼容性、界面和使用问题。提交反馈时请说明 WordPress 版本、PHP 版本、主题名称，以及是否使用 FTPS 或 SFTP；不要附上邮箱密码、付款信息或完整公开链接。
